...
server <- function(input, output, session) {
  ...
  generate_plot_and_model <- function() {
    ...
    # Calculate LoD, LoQ, and LoL values
    lod_x <- 0.05
    loq_x <- 0.1
    lol_x <- 1.5
    ...
    list(
      plot = function() {
        ...
        # Draw LoD, LoQ, LoL lines on plot
      ...
      # Return LoD, LoQ, LoL values for use in original plot
      lod_x = lod_x, loq_x = loq_x, lol_x = lol_x,
      lod_y = lod_y, loq_y = loq_y, lol_y = lol_y
  ...
  output$downloadOriginalPlot <- downloadHandler(
    ...
    content = function(file) {
      ...
      # Download values LoD, LoQ i LoL from generate_plot_and_model()
      lod_x <- generate_plot_and_model()$lod_x
      loq_x <- generate_plot_and_model()$loq_x
      lol_x <- generate_plot_and_model()$lol_x
      lod_y <- generate_plot_and_model()$lod_y
      loq_y <- generate_plot_and_model()$loq_y
      lol_y <- generate_plot_and_model()$lol_y
      
      # Draw LoD, LoQ & LoL on original plot
      abline(v = lod_x, col = "red", lty = 2)
      abline(v = loq_x, col = "red", lty = 2)
      abline(v = lol_x, col = "red", lty = 2)
      abline(h = lod_y, col = "blue", lty = 3)
