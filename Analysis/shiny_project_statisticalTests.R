...
########################        Define UI       ########################
ui <- fluidPage(
  # Layout: Keep it as in the original code, just add extra inputs
  ...
  # New section for statistical tests
  fluidRow(
    column(12, h3("Additional Statistical Tests"))
  ),
  fluidRow(
    column(4,
           selectInput("corTest", "Correlation Test",
                       choices = c("Pearson" = "pearson", "Spearman" = "spearman"),
                       selected = "pearson"),
           actionButton("runCor", "Run Correlation Test"),
           verbatimTextOutput("corTestOutput")
    ),
    column(4,
           selectInput("anovaFactor", "ANOVA Factor:",
                       choices = c("T.F"), # adjust if there are other grouping variables
                       selected = "T.F"),
           actionButton("runAnova", "Run ANOVA"),
           verbatimTextOutput("anovaOutput")
    ),
    column(4,
           selectInput("tTestGroup", "Group for t-test",
                       choices = c("T.F"), # Ensure this is a factor with two levels
                       selected = "T.F"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$runCor, {
    mod_data <- generate_plot_and_model()$data
    # Correlate STD..ng. and PEAK
    cor_res <- cor.test(mod_data$STD..ng., mod_data$PEAK, method = input$corTest)
    output$corTestOutput <- renderPrint({
      cat("Correlation Test (", input$corTest, "):\n", sep = "")
      print(cor_res)
    })
  })
  ...
  observeEvent(input$runAnova, {
    # Example ANOVA: PEAK ~ factor (like T.F) using original data
    if (input$anovaFactor %in% colnames(data)) {
      aov_res <- aov(PEAK ~ as.factor(data[[input$anovaFactor]]), data = data)
      ...
      output$anovaOutput <- renderPrint({
        cat("Selected factor not found in the data.")
      })
    }
  })
  ...
  observeEvent(input$runTtest, {
    # Assuming the chosen factor has two levels
    if (input$tTestGroup %in% colnames(data)) {
      factor_col <- data[[input$tTestGroup]]
      unique_groups <- unique(factor_col)
      if (length(unique_groups) == 2) {
        group1 <- data$PEAK[factor_col == unique_groups[1]]
        group2 <- data$PEAK[factor_col == unique_groups[2]]
        ...
        output$tTestOutput <- renderPrint({
          cat("Selected factor does not have exactly two levels. Cannot perform t-test.")
        })
      }
    } else {
      output$tTestOutput <- renderPrint({
  ...
  # Save objects after the app closes
  onStop(function() {
    # Save the filtered model and data to an RDS file
