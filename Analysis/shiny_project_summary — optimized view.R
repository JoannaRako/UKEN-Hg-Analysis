...
server <- function(input, output, session) {
  ...
  generate_plot_and_model <- function() {
    ...
    # Granica wykrywalności (LOD – Limit of Detection)
    # Dolna granica oznaczalności (LLOQ – Lower Limit of Quantification)
    
    lod_x <- 0.05; loq_x <- 0.1; lol_x <- 1.5
  ...
  output$downloadOriginalPlot <- downloadHandler(
    ...
    content = function(file) {
      ...
      lod_y <- predict(if (input$modelType == "poly") poly_original else linear_original, data.frame(STD..ng. = 0.05))
      loq_y <- predict(if (input$modelType == "poly") poly_original else linear_original, data.frame(STD..ng. = 0.1))
      lol_y <- predict(if (input$modelType == "poly") poly_original else linear_original, data.frame(STD..ng. = 1.5))
      abline(h = lod_y, col = "blue", lty = 3)
      abline(h = loq_y, col = "blue", lty = 3)
      abline(h = lol_y, col = "blue", lty = 3)
      text(0.05, lod_y, "LoD", col = "red", pos = 4)
      text(0.1, loq_y, "LoQ", col = "red", pos = 4)
      text(1.5, lol_y, "LoL", col = "red", pos = 4)
