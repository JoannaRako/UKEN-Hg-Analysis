...
server <- function(input, output, session) {
  ...
  observeEvent(input$plot_click, {
    ...
    new_point <- which.min((true_rows_df$STD..ng. - click$x)^2 + (true_rows_df$PEAK - click$y)^2)
  ...
  output$calibrationPlot <- renderPlot({
    ...
    true_rows_df$included <- TRUE
    true_rows_df$included[outliers] <- FALSE
    
    included_rows <- true_rows_df[true_rows_df$included, ]
    excluded_rows <- true_rows_df[!true_rows_df$included, ]
    ...
    plot(included_rows$STD..ng., included_rows$PEAK, # True rowa
    ...
    legend("topleft",
           legend=c("Included (True)", "Excluded", "False"),
