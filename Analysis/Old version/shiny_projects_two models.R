...
ui <- fluidPage(
  ...
  fluidRow(
    column(12, 
           ...
           actionButton("switchToLinear", "Switch to Linear Model"),
           actionButton("switchToPoly", "Switch to Polynomial 3rd Degree Model"),
    )
  ),
...
server <- function(input, output, session) {
  ...
  # Reactive value to track the model type
  model_type <- reactiveVal("poly")
  ...
  # Observe model switch button clicks
  observeEvent(input$switchToLinear, {
    model_type("linear")
  })
  
  observeEvent(input$switchToPoly, {
    model_type("poly")
  })
    ...
    list(
      plot = function() {
        plot(true_points$STD..ng., true_points$PEAK, 
             main = ifelse(model_type() == "poly", "Calibration Curve - Polynomial 3rd Degree", "Calibration Curve - Linear Model"),
             xlab = "STD..ng.", 
             ylab = "PEAK", 
             pch = 16,
             col = "green",
  ...
  output$downloadOriginalPlot <- downloadHandler(
    ...
    content = function(file) {
      ...
      plot(data$STD..ng.[data$T.F == "True"], data$PEAK[data$T.F == "True"], 
           main = ifelse(model_type() == "poly", "Original Calibration Curve - Polynomial 3rd Degree", "Original Calibration Curve - Linear Model"),
           xlab = "STD..ng.", 
           ylab = "PEAK", 
           pch = 16,
           col = "green",
